...
class VoxelModel():
    ...
    def color_block_left_and_right(self, j, k, blockSize, color, color2, numColor):  # i = width, j = height, k = layer
        numLayers = len(self.Z) - 1
            ...
            if k + inc_k < numLayers:
                for inc_j in range(blockSize):
                    if j + inc_j < self.height and True in self.blocks[k + inc_k][j + inc_j]:
                        ...
                        self.colors[k + inc_k][j + inc_j][i] = color
                        self.colors[k + inc_k][j + inc_j][i_invert] = color
                        for inc_i in range(1, blockSize):
                            if i + inc_i < self.width and self.blocks[k + inc_k][j + inc_j][i + inc_i]==True:
                                if self.colors[k + inc_k][j + inc_j][i + inc_i] == numColor+1:
                                    self.colors[k + inc_k][j + inc_j][i + inc_i] = color
                            if i_invert - inc_i > 0 and self.blocks[k + inc_k][j + inc_j][i_invert - inc_i]==True:
                                if self.colors[k + inc_k][j + inc_j][i_invert - inc_i] == numColor+1:
                                    self.colors[k + inc_k][j + inc_j][i_invert - inc_i] = color
        return color

    def color_block_front_and_back(self, i, k, blockSize, color, color2, numColor):  # i = width, j = height, k = layer
        numLayers = len(self.Z) - 1
        for inc_k in range(blockSize):
            if k + inc_k < numLayers:
                for inc_i in range(blockSize):
                    if i + inc_i < self.width:
                        ...
                        if True in column_i:
                            ...
                            for inc_j in range(1,blockSize):
                                if j + inc_j < self.height and self.blocks[k + inc_k][j + inc_j][i + inc_i]==True:
                                    if self.colors[k + inc_k][j + inc_j][i + inc_i] == numColor+1:
                                        self.colors[k + inc_k][j + inc_j][i + inc_i] = color
                                if j_invert - inc_j > 0 and self.blocks[k + inc_k][j_invert - inc_j][i + inc_i]==True:
                                    if self.colors[k + inc_k][j_invert - inc_j][i + inc_i] == numColor+1:
                                        self.colors[k + inc_k][j_invert - inc_j][i + inc_i] = color
        return color

    def color_block_top(self,i,j,blockSize, color, numColor):
        numLayers = len(self.Z) - 1
        for inc_j in range(blockSize):
                ...
                for inc_i in range(blockSize):
                    if i + inc_i < self.width:
                        ...
                        if True in column_k:
                            ...
                            self.colors[k][j + inc_j][i + inc_i] = color
                            for inc_k in range(1, blockSize):
                                if k-inc_k > 0 and self.blocks[k-inc_k][j+inc_j][i+inc_i]==True:
                                    if self.colors[k - inc_k][j + inc_j][i + inc_i] == numColor+1:
                                        self.colors[k - inc_k][j + inc_j][i + inc_i] = color


    def determine_color_blocks(self, numColor, blockSize):
        numLayers = len(self.Z) - 1
        ...
        for k in range(0, numLayers, blockSize):
            ...
            for i in range(0, self.width, blockSize):
                ...
                #color = (k_prime * (self.width // blockSize) + i_prime) % numColor + 1
                #color2 = (k_prime * (self.width // blockSize) + i_prime + 1) % numColor + 1
                self.color_block_front_and_back(i, k, blockSize, color+1, color+1, numColor)
                if i == 0:
                    first_color = color
        ...
        for k in range(0, numLayers, blockSize):
            ...
            for j in range(0, self.height, blockSize):
                k_prime = k//blockSize
                j_prime = j//blockSize
                #color = (k_prime*(self.height//blockSize)+j_prime)%numColor+1
                #color2 = (k_prime*(self.height//blockSize)+j_prime+1)%numColor+1
                color = (color + 1) % numColor
                self.color_block_left_and_right(j,k,blockSize, color+1, color+1, numColor)
                if j == 0:
                    first_color = color
        ...
        for j in range(0, self.height, blockSize):
            ...
            for i in range(0, self.width, blockSize):
                ...
                i_prime = i // blockSize
                color = (color + 1) % numColor
                self.color_block_top(i, j, blockSize, color+1, numColor)
                if i == 0:
                    first_color = color
